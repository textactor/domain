...
test("#endsWithNumberWord", (t) => {
  t.is(
    NameHelper.endsWithNumberWord("iPhone6"),
    false,
    "iPhone6 is not ending with a number word"
  );
  t.is(
    NameHelper.endsWithNumberWord("iPhone 6"),
    true,
    "iPhone 6 is ending with a number word"
  );
});
...
test("#removeSymbols", (t) => {
  ...
  t.is(
    NameHelper.removeSymbols("iPhone $^@^%*#^*(#()*#_*_)(@_)(@ &+-iPad"),
    "iPhone&iPad"
  );
});
